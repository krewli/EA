...
namespace knapsack
{
	...
	TEST_F( KnapsackSelectionTestSuite, test_selection_with_only_one_individual_having_fitness_above_0 )
	{
		...
		util::Seed seed { util::NO_SEED };

		knapsack_individual id1 { 3 };
		knapsack_individual id2 { 3 };
		knapsack_individual id3 { 3 };

		knapsack_individual::chromosome_encoding chromosome1 { true, true, true };
		knapsack_individual::chromosome_encoding chromosome2 { false, false, false };
		knapsack_individual::chromosome_encoding chromosome3 { false, false, false };
		...
		EXPECT_EQ(id1, actual_result);
		EXPECT_NE(id2, actual_result);
		EXPECT_NE(id3, actual_result);
	    }
	    catch ( std::runtime_error& e )
	    {
